...
CREATE TABLE tblOneInquiry (
    ...
    qusetionTitle VARCHAR2(300) NOT NULL, --질문제목
...
create table tblJobAdvertisement(
    ...
    storageStatus number default 0 not null check(storageStatus in(0, 1)), --저장상태(작성중 = 0, 작성완료 = 1)
...
create table tblResume( -- 이력서
    ...
    storageStatus number default 0 not null check(storageStatus in(0, 1)), -- 저장상태
